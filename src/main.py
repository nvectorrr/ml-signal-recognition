import numpy as np
from src.lib.signals_generator import generate_ofdm_signal, generate_ofdm_signal_bpsk, generate_am_signal, generate_fm_signal, generate_qam_signal
from src.lib.ml import evaluate_svm, evaluate_random_forest, evaluate_knn
from src.lib.plots import plot_ofdm_signal, plot_ofdm_bpsk_signal, plot_am_signal, plot_fm_signal, plot_qam_signal, plot_predictions

# generate and plot signals
ofdm_data = generate_ofdm_signal()
plot_ofdm_signal(ofdm_data)

ofdm_bpsk_data = generate_ofdm_signal_bpsk()
plot_ofdm_bpsk_signal(ofdm_bpsk_data)
...
X = np.concatenate([ofdm_data, am_data, fm_data, qam_data, ofdm_bpsk_data])
y = np.array([1] * len(ofdm_data) + [2] * len(am_data) + [3] * len(fm_data) + [4] * len(qam_data) + [5] * len(ofdm_bpsk_data))  # 1 - OFDM, 2 - AM, 3 - FM, 4 - QAM, 5 - OFDM (BPSK)
